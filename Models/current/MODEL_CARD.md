...
- Numeric fixtures show the dictionary values are finite and not normalized; their argmax selects `classLabel`. This proves only an observed `raw classifier scores` boundary. Whether those values are logits or another raw-score representation remains unknown until a deterministic checksum-bound graph extractor or the source/export graph is available. Stable softmax is serialized separately.
...
3. `classLabel_probs` contains observed non-normalized raw classifier scores of unresolved kind, but the wrapper applies a stable softmax before returning `softmax(LABEL_2) − softmax(LABEL_0)`.
...
7. The unchanged Swift runtime uses Core ML's default unrestricted compute policy. The reference verifier pins CPU-only execution for independent raw-score evidence and separately runs the production `BertModelWrapper` with its default policy against committed reference transform values under its own fresh-process calibrated tolerance.

Golden fixtures serialize the independent numeric transform for parity but mark final semantic mapping unknown. The production runtime is compared directly with that committed value; its expected value is not reconstructed inside the runtime-parity test. This task intentionally does not change runtime sources.
...
Calibration uses the complete corpus in at least ten fresh sequential OS processes. A later set of at least five fresh processes must remain inside the fixed maximum observed calibration delta. Separate evidence is recorded for CPU-only reference outputs and the real production `BertModelWrapper` under its default compute policy. Raw process values, recomputed distributions, environment, compute policy, backend, and holdout decisions are stored in `Tests/Golden/current/current-russian-fixture-v1/calibration-v1.json`.

Public CI uses macOS 26, Xcode 26.6, and an explicitly installed Python 3.14.0 to validate schema/comparator behavior and the checksum closure of `verification-v1.json`. Protected assets remain outside GitHub-hosted CI. Until a protected runner exists, that attestation binds the completed local asset-backed commands to exact model, vocabulary, production source, tooling, manifest, fixture, calibration, and environment checksums.
...
The runtime-refactor gate is blocked while any of label semantics, output kind, training-tokenizer parity, or provenance is unresolved. All four conditions currently remain unresolved.
